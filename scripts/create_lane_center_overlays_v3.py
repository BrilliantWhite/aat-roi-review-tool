#!/usr/bin/env python3
"""Create original-image overlays for v3 lane-center candidates."""
...
CANDIDATES_PATH = PROJECT_ROOT / "outputs" / "lane_segmentation" / "lane_centers_v3" / "lane_center_candidates_v3.csv"
CENTER_MANIFEST_PATH = PROJECT_ROOT / "outputs" / "lane_segmentation" / "lane_centers_v3" / "lane_center_manifest_v3.csv"
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "lane_segmentation" / "overlays" / "centerlines_v3"
OVERLAY_MANIFEST_PATH = OUTPUT_DIR / "lane_center_overlay_manifest_v3.csv"

DETECTED_LINE_COLOR = (220, 20, 60, 255)
HIGH_FALLBACK_LINE_COLOR = (42, 157, 143, 255)
MEDIUM_FALLBACK_LINE_COLOR = (247, 127, 0, 255)
LOW_FALLBACK_LINE_COLOR = (108, 117, 125, 255)
DETECTED_BAND_COLOR = (255, 215, 0, 42)
HIGH_FALLBACK_BAND_COLOR = (42, 157, 143, 36)
MEDIUM_FALLBACK_BAND_COLOR = (255, 165, 0, 34)
...
def draw_label(draw: ImageDraw.ImageDraw, image_id: str, center_count: int, accepted_count: int, fallback_count: int, notes: str) -> None:
    label = f"{image_id}: {accepted_count}/{center_count} v3 centerlines ({fallback_count} fallback); diagnostic guide only"
    if notes:
        label = f"{label}; {notes}"
    try:
...
def colors_for_candidate(row: dict[str, str]) -> tuple[tuple[int, int, int, int], tuple[int, int, int, int]]:
    source = row.get("source", "detected_peak")
    confidence = row.get("confidence", "high")
    status = row.get("status", "accepted")
    if source == "detected_peak":
        return DETECTED_BAND_COLOR, DETECTED_LINE_COLOR
    if status == "review_only" or confidence == "low":
        return LOW_FALLBACK_BAND_COLOR, LOW_FALLBACK_LINE_COLOR
    if confidence == "high":
        return HIGH_FALLBACK_BAND_COLOR, HIGH_FALLBACK_LINE_COLOR
    return MEDIUM_FALLBACK_BAND_COLOR, MEDIUM_FALLBACK_LINE_COLOR
...
def create_overlay(image_id: str, candidate_rows: list[dict[str, str]], manifest_row: dict[str, str]) -> Path:
    ...
    draw_label(draw, image_id, len(candidate_rows), accepted_count, fallback_count, notes)

    output = Image.alpha_composite(base, overlay).convert("RGB")
    output_path = OUTPUT_DIR / f"{image_id}_lane_center_overlay_v3.png"
...
def main() -> None:
    ...
    for record in records:
        ...
        notes = "v3 conditional centerlines; red=direct peak, green/orange=accepted fallback, grey=review-only fallback; not final lane boundaries"
    ...
    print(f"Wrote {len(manifest_rows)} v3 lane-center overlays to {OUTPUT_DIR}")
