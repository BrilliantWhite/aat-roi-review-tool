#!/usr/bin/env python3
"""Create original-image overlays for v6_1 horizontal-ROI lane-boundary candidates."""
...
CANDIDATES_PATH = PROJECT_ROOT / "outputs" / "lane_segmentation" / "lane_roi_boundaries_v6_1" / "lane_boundary_candidates_v6_1.csv"
BOUNDARY_MANIFEST_PATH = PROJECT_ROOT / "outputs" / "lane_segmentation" / "lane_roi_boundaries_v6_1" / "lane_boundary_manifest_v6_1.csv"
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "lane_segmentation" / "overlays" / "roi_boundaries_v6_1"
OVERLAY_MANIFEST_PATH = OUTPUT_DIR / "lane_roi_boundary_overlay_manifest_v6_1.csv"
...
def draw_label(draw: ImageDraw.ImageDraw, image_id: str, boundary_count: int, accepted_count: int, median_width: str, notes: str) -> None:
    label = f"{image_id}: {accepted_count}/{boundary_count} v6_1 adaptive lane boundaries; median width={median_width or 'n/a'} px"
    if notes:
        label = f"{label}; {notes}"
    try:
...
def create_overlay(image_id: str, candidate_rows: list[dict[str, str]], manifest_row: dict[str, str]) -> Path:
    ...
    output_path = OUTPUT_DIR / f"{image_id}_lane_boundary_overlay_v6_1.png"
...
def main() -> None:
    ...
    for record in records:
        ...
        notes = "v6_1 ROI boundary overlay; green band=shared horizontal ROI, yellow span=estimated lane coverage clipped to ROI, yellow edges=estimated boundaries, red/orange/green/grey=center status"
    ...
    print(f"Wrote {len(manifest_rows)} v6_1 lane-boundary overlays to {OUTPUT_DIR}")
