#!/usr/bin/env python3
"""Detect standalone v5 adaptive lane boundary candidates for raw gel images."""
...
CONFIG_PATH = PROJECT_ROOT / "configs" / "lane_boundary_detection_v5.yaml"
LANE_COUNT_REVIEW_PATH = PROJECT_ROOT / "dataset" / "metadata" / "lane_count_review.csv"
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "lane_segmentation" / "lane_boundaries_v5"
CANDIDATES_PATH = OUTPUT_DIR / "lane_boundary_candidates_v5.csv"
MANIFEST_PATH = OUTPUT_DIR / "lane_boundary_manifest_v5.csv"
COMPARISON_PATH = OUTPUT_DIR / "lane_boundary_v4_v5_comparison.csv"
PLOTS_DIR = OUTPUT_DIR / "diagnostics"
V4_MANIFEST_PATH = PROJECT_ROOT / "outputs" / "lane_segmentation" / "lane_centers_v4" / "lane_center_manifest_v4.csv"
...
def make_diagnostic_plot(image_id: str, whole_profile: np.ndarray, detection_data: dict[str, object], boundaries: list[dict[str, object]], projection_row: dict[str, str]) -> Path:
    output_path = PLOTS_DIR / f"{image_id}_lane_boundary_candidates_v5.png"
    ...
    ax.plot(x_values, detection_data["smoothed"], color="#4b2e83", linewidth=1.4, label="v5 combined smoothed")
    ...
    ax.set_title(f"{image_id} v5 adaptive lane boundaries ({projection_row['quality_flag']}: {projection_row['source_filename']})")
...
def candidate_rows(image_id: str, whole_profile: np.ndarray, centers: np.ndarray, detection_data: dict[str, object], boundaries: list[dict[str, object]], method: str) -> list[dict[str, str]]:
    rows = []
...
def main() -> None:
    ...
    v4_manifest = load_csv_by_image_id(V4_MANIFEST_PATH) if V4_MANIFEST_PATH.exists() else {}
    ...
    for record in records:
        ...
        all_candidate_rows.extend(candidate_rows(record.image_id, whole_profile, centers, detection_data, boundaries, method))
        ...
        manifest_rows.append(
            {
                ...
                "method_notes": "standalone v5 detector using raw images and band-region projections; lane boundaries are adaptive valley/gradient estimates around detected centers",
        ...
        if v4_manifest:
            v4_row = v4_manifest.get(record.image_id, {})
            v4_count = int(v4_row.get("accepted_center_count", 0) or 0)
            comparison_rows.append(
                {
                    "image_id": record.image_id,
                    "source_filename": record.source_filename,
                    "quality_flag": projection_row["quality_flag"],
                    "v4_center_count": str(v4_count),
                    "v5_boundary_count": str(len(accepted_centers)),
                    "boundary_count_delta": str(len(accepted_centers) - v4_count),
                    "v4_detection_quality_flag": v4_row.get("detection_quality_flag", ""),
                    "v5_detection_quality_flag": quality,
                    "v5_median_estimated_width_px": median_width,
                    "fallback_allowed_v5": str(bool(detection_data["fallback_allowed"])).lower(),
                    "fallback_reason_v5": str(detection_data["fallback_reason"]),
                }
            )
    ...
    print(f"Wrote {len(all_candidate_rows)} v5 lane-boundary candidates for {len(manifest_rows)} images")
    ...
    if comparison_rows:
        ...
        print(f"Wrote optional v4-v5 comparison to {COMPARISON_PATH}; count changed for {changed_count} images")
    else:
        print("Skipped optional v4-v5 comparison because v4 manifest was not found")
