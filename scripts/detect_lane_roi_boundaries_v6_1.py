#!/usr/bin/env python3
"""Detect standalone v6_1 horizontal-ROI-aware lane boundary candidates."""
...
CONFIG_PATH = PROJECT_ROOT / "configs" / "lane_roi_boundary_detection_v6_1.yaml"
LANE_COUNT_REVIEW_PATH = PROJECT_ROOT / "dataset" / "metadata" / "lane_count_review.csv"
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "lane_segmentation" / "lane_roi_boundaries_v6_1"
CANDIDATES_PATH = OUTPUT_DIR / "lane_boundary_candidates_v6_1.csv"
MANIFEST_PATH = OUTPUT_DIR / "lane_boundary_manifest_v6_1.csv"
COMPARISON_PATH = OUTPUT_DIR / "lane_boundary_v6_v6_1_comparison.csv"
ROI_MANIFEST_PATH = OUTPUT_DIR / "horizontal_roi_manifest_v6_1.csv"
PLOTS_DIR = OUTPUT_DIR / "diagnostics"
V6_MANIFEST_PATH = PROJECT_ROOT / "outputs" / "lane_segmentation" / "lane_roi_boundaries_v6" / "lane_boundary_manifest_v6.csv"
...
COMPARISON_FIELDNAMES = [
    ...
    "v6_detection_quality_flag",
    "v6_1_detection_quality_flag",
    "v6_1_median_estimated_width_px",
    "fallback_allowed_v6_1",
    "fallback_reason_v6_1",
]
...
def segment_score(profile: np.ndarray, start: int, end: int, config: dict[str, object]) -> float:
    height = max(1, end - start)
    mean_signal = float(np.mean(profile[start:end]))
    peak_signal = float(np.max(profile[start:end]))
    height_weight = height ** float(config["roi_segment_height_power"])
    return height_weight * (mean_signal + peak_signal * float(config["roi_segment_peak_weight"]))


def trim_header_and_numbers(profile: np.ndarray, y_start: int, y_end: int, height: int, config: dict[str, object]) -> tuple[int, int, list[str]]:
    reasons: list[str] = []
    header_limit = round(height * float(config["roi_header_exclusion_fraction"]))
    min_start = round(height * float(config["roi_allowed_y_min_fraction"]))
    if y_start < header_limit:
        scan_end = min(y_end, header_limit + round(height * float(config["roi_header_scan_extra_fraction"])))
        if scan_end > y_start + 2:
            local = profile[y_start:scan_end]
            cutoff = max(float(np.quantile(local, float(config["roi_header_low_signal_quantile"]))) if len(local) else 0.0, float(config["roi_header_low_signal_floor"]))
            low_rows = np.where(local <= cutoff)[0]
            lower_rows = low_rows[y_start + low_rows >= header_limit]
            if len(lower_rows):
                y_start = max(min_start, y_start + int(lower_rows[0]))
                reasons.append("header_suppressed")
            else:
                y_start = max(y_start, header_limit)
                reasons.append("header_floor_applied")
    bottom_limit = round(height * float(config["roi_bottom_number_exclusion_fraction"]))
    if y_end > bottom_limit:
        scan_start = max(y_start, bottom_limit - round(height * float(config["roi_bottom_number_scan_extra_fraction"])))
        if y_end > scan_start + 2:
            local = profile[scan_start:y_end]
            cutoff = max(float(np.quantile(local, float(config["roi_bottom_low_signal_quantile"]))) if len(local) else 0.0, float(config["roi_bottom_low_signal_floor"]))
            low_rows = np.where(local <= cutoff)[0]
            upper_rows = low_rows[scan_start + low_rows <= bottom_limit]
            if len(upper_rows):
                y_end = min(y_end, scan_start + int(upper_rows[-1]) + 1)
                reasons.append("bottom_numbers_suppressed")
            else:
                y_end = min(y_end, bottom_limit)
                reasons.append("bottom_floor_applied")
    return y_start, y_end, reasons
...
def estimate_horizontal_roi(image_id: str, whole_profile: np.ndarray, config: dict[str, object]) -> dict[str, object]:
    ...
    smoothed = smooth_profile(row_profile, odd_window_size_for_length(height, config, "roi_smoothing_window_fraction", "roi_minimum_smoothing_window_px", "roi_maximum_smoothing_window_px"))
    ...
    broad_start, broad_end = int(y_start), int(y_end)
    broad_height = max(1, broad_end - broad_start)
    roi = {"y_start": broad_start, "y_end": broad_end, "roi_height": broad_height}
    centers = preliminary_lane_centers_for_roi(image_id, config, roi)
    lane_support = 0
    lane_reason = "no_lane_evidence"
    if len(centers):
        spacing = float(np.median(np.diff(np.sort(centers)))) if len(centers) > 1 else width / max(2, int(config["coverage_target_lane_count"]))
        ...
        for center_x in centers:
            ...
            if right > left:
                lane_profiles.append(normalize(inverted[:, left:right].mean(axis=1)))
        if lane_profiles:
            lane_stack = np.vstack(lane_profiles)
            aggregate = normalize(0.70 * normalize(np.quantile(lane_stack, float(config["roi_lane_profile_quantile"]), axis=0)) + 0.30 * smoothed)
            local = aggregate[broad_start:broad_end]
            local_peak = float(np.max(local)) if len(local) else 0.0
            lane_threshold = max(float(np.quantile(local, float(config["roi_lane_signal_threshold_quantile"]))) if len(local) else 0.0, local_peak * float(config["roi_lane_signal_threshold_floor_fraction"]))
            mask = local >= lane_threshold
            support = np.mean(lane_stack[:, broad_start:broad_end] >= lane_threshold, axis=0)
            min_support = max(float(config["roi_lane_evidence_min_support_fraction"]), int(config["roi_lane_evidence_min_support_count"]) / max(1, len(lane_profiles)))
            supported = mask & (support >= min_support)
            if not np.any(supported):
                supported = mask
            if np.any(supported):
                indices = np.where(supported)[0]
                lower_min = round(broad_height * float(config["roi_lower_retention_min_fraction"]))
                lower_support = mask & (support >= float(config["roi_lower_retention_min_support_fraction"]))
                lower_indices = np.where(lower_support & (np.arange(len(local)) >= lower_min))[0]
                if len(lower_indices):
                    indices = np.concatenate([indices, np.array([int(np.quantile(lower_indices, float(config["roi_lower_retention_quantile"])))])])
                lane_start = broad_start + int(np.quantile(indices, float(config["roi_lane_upper_quantile"])))
                lane_end = broad_start + int(np.quantile(indices, float(config["roi_lane_lower_quantile"]))) + 1 + round(height * float(config["roi_lower_retention_padding_fraction"]))
                lane_support = len(lane_profiles)
                lane_reason = "lane_evidence_aggregated"
                if lane_end - lane_start >= round(height * float(config["roi_lane_evidence_min_height_fraction"])):
                    max_shift = round(height * float(config["roi_refinement_max_shift_fraction"]))
                    lane_start = max(broad_start - max_shift, min(lane_start, broad_start + max_shift))
                    lane_end = max(broad_end - max_shift, min(lane_end, broad_end + max_shift))
                    overlap = max(0, min(lane_end, broad_end) - max(lane_start, broad_start)) / broad_height
                    if overlap >= float(config["roi_refinement_min_overlap_fraction"]):
                        y_start, y_end = int(lane_start), int(lane_end)
                    else:
                        lane_reason = "lane_evidence_low_overlap_fallback_to_v6"
                else:
                    lane_reason = "lane_evidence_too_thin_fallback_to_v6"
    bottom_guard = round(height * float(config["roi_bottom_guard_fraction"]))
    if y_end > bottom_guard:
        y_end = bottom_guard
        reason = f"{reason};bottom_guard_applied"
    if y_end - y_start < round(height * float(config["roi_fallback_min_height_fraction"])):
        y_start, y_end = broad_start, broad_end
        reason = f"{reason};fallback_to_v6_broad_roi_too_thin"
    roi_height = y_end - y_start
    height_fraction = roi_height / height if height else 0.0
    if peak_value < 0.05 or height_fraction < float(config["roi_low_confidence_height_fraction"]):
        confidence = "low"
        quality = "review"
    elif lane_support >= int(config["roi_lane_evidence_min_support_count"]):
        confidence = "high"
        quality = "ok"
    else:
        confidence = "medium"
    ...
    return {
        ...
        "roi_reason": f"{reason};{lane_reason};lane_support={lane_support}",
...
def make_diagnostic_plot(image_id: str, whole_profile: np.ndarray, detection_data: dict[str, object], boundaries: list[dict[str, object]], projection_row: dict[str, str]) -> Path:
    output_path = PLOTS_DIR / f"{image_id}_lane_boundary_candidates_v6_1.png"
    ...
    ax.plot(x_values, detection_data["smoothed"], color="#4b2e83", linewidth=1.4, label="v6_1 combined smoothed")
    ...
    ax.set_title(f"{image_id} v6_1 adaptive lane boundaries ({projection_row['quality_flag']}: {projection_row['source_filename']})")
...
def main() -> None:
    ...
    v6_manifest = load_csv_by_image_id(V6_MANIFEST_PATH) if V6_MANIFEST_PATH.exists() else {}
    ...
    for record in records:
        ...
        manifest_rows.append(
            {
                ...
                "method_notes": "standalone v6_1 detector using raw images, shared metadata, and one horizontal ROI per image; lane boundaries are adaptive valley/gradient estimates around ROI-limited detected centers",
        ...
        if v6_manifest:
            v6_row = v6_manifest.get(record.image_id, {})
            v6_count = int(v6_row.get("accepted_boundary_count", 0) or 0)
            comparison_rows.append(
                {
                    "image_id": record.image_id,
                    "source_filename": record.source_filename,
                    "quality_flag": projection_row["quality_flag"],
                    "v6_boundary_count": str(v6_count),
                    "v6_1_boundary_count": str(len(accepted_centers)),
                    "boundary_count_delta": str(len(accepted_centers) - v6_count),
                    "v6_detection_quality_flag": v6_row.get("detection_quality_flag", ""),
                    "v6_1_detection_quality_flag": quality,
                    "v6_1_median_estimated_width_px": median_width,
                    "fallback_allowed_v6_1": str(bool(detection_data["fallback_allowed"])).lower(),
                    "fallback_reason_v6_1": str(detection_data["fallback_reason"]),
                }
            )
    ...
    print(f"Wrote {len(all_candidate_rows)} v6_1 lane-boundary candidates for {len(manifest_rows)} images")
    ...
    if comparison_rows:
        ...
        print(f"Wrote optional v6-v6_1 comparison to {COMPARISON_PATH}; count changed for {changed_count} images")
    else:
        print("Skipped optional v6-v6_1 comparison because v6 manifest was not found")
