#!/usr/bin/env python3
"""Detect standalone v6 horizontal-ROI-aware lane boundary candidates."""
...
CONFIG_PATH = PROJECT_ROOT / "configs" / "lane_roi_boundary_detection_v6.yaml"
LANE_COUNT_REVIEW_PATH = PROJECT_ROOT / "dataset" / "metadata" / "lane_count_review.csv"
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "lane_segmentation" / "lane_roi_boundaries_v6"
CANDIDATES_PATH = OUTPUT_DIR / "lane_boundary_candidates_v6.csv"
MANIFEST_PATH = OUTPUT_DIR / "lane_boundary_manifest_v6.csv"
COMPARISON_PATH = OUTPUT_DIR / "lane_boundary_v5_v6_comparison.csv"
ROI_MANIFEST_PATH = OUTPUT_DIR / "horizontal_roi_manifest_v6.csv"
PLOTS_DIR = OUTPUT_DIR / "diagnostics"
V5_MANIFEST_PATH = PROJECT_ROOT / "outputs" / "lane_segmentation" / "lane_boundaries_v5" / "lane_boundary_manifest_v5.csv"
...
COMPARISON_FIELDNAMES = [
    ...
    "v5_detection_quality_flag",
    "v6_detection_quality_flag",
    "v6_median_estimated_width_px",
    "fallback_allowed_v6",
    "fallback_reason_v6",
]
...
def estimate_horizontal_roi(image_id: str, config: dict[str, object]) -> dict[str, object]:
    ...
    if np.any(mask):
        ...
        reason = "row_signal_threshold_band"
    else:
        ...
        reason = "fallback_centered_minimum_height"
    ...
    if peak_value < 0.05 or height_fraction < float(config["roi_low_confidence_height_fraction"]):
        confidence = "low"
        quality = "review"
    elif reason.endswith("clamped_to_max_height") or "fallback" in reason:
        confidence = "medium"
        quality = "review"
    else:
        confidence = "high"
...
def make_diagnostic_plot(image_id: str, whole_profile: np.ndarray, detection_data: dict[str, object], boundaries: list[dict[str, object]], projection_row: dict[str, str]) -> Path:
    output_path = PLOTS_DIR / f"{image_id}_lane_boundary_candidates_v6.png"
    ...
    ax.plot(x_values, detection_data["smoothed"], color="#4b2e83", linewidth=1.4, label="v6 combined smoothed")
    ...
    ax.set_title(f"{image_id} v6 adaptive lane boundaries ({projection_row['quality_flag']}: {projection_row['source_filename']})")
...
def main() -> None:
    ...
    v5_manifest = load_csv_by_image_id(V5_MANIFEST_PATH) if V5_MANIFEST_PATH.exists() else {}
    ...
    for record in records:
        ...
        roi = estimate_horizontal_roi(record.image_id, config)
        roi_rows.append(
            {
        ...
        manifest_rows.append(
            {
                ...
                "method_notes": "standalone v6 detector using raw images, shared metadata, and one horizontal ROI per image; lane boundaries are adaptive valley/gradient estimates around ROI-limited detected centers",
        ...
        if v5_manifest:
            v5_row = v5_manifest.get(record.image_id, {})
            v5_count = int(v5_row.get("accepted_boundary_count", 0) or 0)
            comparison_rows.append(
                {
                    "image_id": record.image_id,
                    "source_filename": record.source_filename,
                    "quality_flag": projection_row["quality_flag"],
                    "v5_boundary_count": str(v5_count),
                    "v6_boundary_count": str(len(accepted_centers)),
                    "boundary_count_delta": str(len(accepted_centers) - v5_count),
                    "v5_detection_quality_flag": v5_row.get("detection_quality_flag", ""),
                    "v6_detection_quality_flag": quality,
                    "v6_median_estimated_width_px": median_width,
                    "fallback_allowed_v6": str(bool(detection_data["fallback_allowed"])).lower(),
                    "fallback_reason_v6": str(detection_data["fallback_reason"]),
                }
            )
    ...
    print(f"Wrote {len(all_candidate_rows)} v6 lane-boundary candidates for {len(manifest_rows)} images")
    ...
    if comparison_rows:
        ...
        print(f"Wrote optional v5-v6 comparison to {COMPARISON_PATH}; count changed for {changed_count} images")
    else:
        print("Skipped optional v5-v6 comparison because v5 manifest was not found")
