#!/usr/bin/env python3
"""Detect v3 candidate vertical lane centers with conditional fallback."""
...
CONFIG_PATH = PROJECT_ROOT / "configs" / "lane_center_detection_v3.yaml"
LANE_COUNT_REVIEW_PATH = PROJECT_ROOT / "dataset" / "metadata" / "lane_count_review.csv"
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "lane_segmentation" / "lane_centers_v3"
CANDIDATES_PATH = OUTPUT_DIR / "lane_center_candidates_v3.csv"
MANIFEST_PATH = OUTPUT_DIR / "lane_center_manifest_v3.csv"
COMPARISON_PATH = OUTPUT_DIR / "lane_center_v2_v3_comparison.csv"
PLOTS_DIR = OUTPUT_DIR / "diagnostics"
V2_MANIFEST_PATH = PROJECT_ROOT / "outputs" / "lane_segmentation" / "lane_centers_v2" / "lane_center_manifest_v2.csv"
...
MANIFEST_FIELDNAMES = [
    ...
    "fallback_allowed",
    "fallback_reason",
    "added_by_conditional_fallback_count",
...
def merge_close_centers(centers: list[int], sources: dict[int, str], min_distance: int, smoothed: np.ndarray) -> tuple[np.ndarray, list[str]]:
    ...
    for cluster in clusters:
        ...
        merged.append(best)
        if sources.get(best) == "detected_peak":
            merged_sources.append("detected_peak")
        elif any(sources.get(center) == "detected_peak" for center in cluster):
            detected = max((center for center in cluster if sources.get(center) == "detected_peak"), key=lambda center: smoothed[center])
            merged[-1] = detected
            merged_sources.append("detected_peak")
        else:
            merged_sources.append("conditional_fallback")
...
def fallback_decision(peaks: np.ndarray, projection_row: dict[str, str], review_row: dict[str, str] | None, config: dict[str, object]) -> tuple[bool, str, int]:
    ...
    if len(peaks) < int(config["coverage_fallback_min_confident_count"]):
        return False, "blocked_by_too_few_direct_peaks", int(config["coverage_target_lane_count"])
    ...
    return True, "full_layout_supported_by_direct_peak_count_and_image_width", int(config["coverage_target_lane_count"])


def add_conditional_fallback(peaks: np.ndarray, smoothed: np.ndarray, config: dict[str, object], edge_margin: int, target_count: int) -> tuple[np.ndarray, list[str]]:
...
def detect_centers(profile: np.ndarray, projection_row: dict[str, str], review_row: dict[str, str] | None, config: dict[str, object]) -> tuple[np.ndarray, dict[str, object]]:
    width = len(profile)
    ...
    fallback_allowed, fallback_reason, target_count = fallback_decision(peaks, projection_row, review_row, config)
    if fallback_allowed:
        centers, sources = add_conditional_fallback(peaks, smoothed, config, edge_margin, target_count)
    else:
        centers = np.array([int(peak) for peak in peaks], dtype=np.int32)
        sources = ["detected_peak"] * len(centers)
...
def make_diagnostic_plot(image_id: str, profile: np.ndarray, smoothed: np.ndarray, centers: np.ndarray, sources: list[str], statuses: list[str], projection_row: dict[str, str]) -> Path:
    output_path = PLOTS_DIR / f"{image_id}_lane_center_candidates_v3.png"
    ...
    for center_x, source, status in zip(centers, sources, statuses):
        if source == "detected_peak":
            color = "#c1121f"
        elif status == "accepted":
            color = "#2a9d8f"
        elif status == "accepted_for_review":
            color = "#f77f00"
        else:
            color = "#6c757d"
        ax.axvline(center_x, color=color, linewidth=1.0, alpha=0.9)
    ax.set_title(f"{image_id} v3 conditional lane-center candidates ({projection_row['quality_flag']}: {projection_row['source_filename']})")
...
def candidate_rows(image_id: str, profile: np.ndarray, centers: np.ndarray, detection_data: dict[str, object], method: str) -> list[dict[str, str]]:
        ...
        rows.append(
            {
                ...
                "confidence": str(confidences[index - 1]),
                "status": str(statuses[index - 1]),
            }
        )
    return rows
...
def main() -> None:
    ...
    v2_manifest = load_csv_by_image_id(V2_MANIFEST_PATH)
    ...
    for record in records:
        ...
        centers, detection_data = detect_centers(profile, projection_row, review_row, config)
        statuses = detection_data["statuses"]
        sources = detection_data["sources"]
        ...
        added_count = sum(1 for source in sources if source == "conditional_fallback")
        ...
        manual_review_required = bool(added_count) or detection_data["fallback_allowed"] is False or projection_row["quality_flag"] != "ok" or review_row is not None
        ...
        if record.image_id in selected_plots:
            plot_path = make_diagnostic_plot(record.image_id, profile, detection_data["smoothed"], centers, sources, statuses, projection_row)
        ...
        manifest_rows.append(
            {
                ...
                "added_by_conditional_fallback_count": str(added_count),
                "accepted_fallback_count": str(accepted_fallback_count),
                "manual_review_required": str(manual_review_required).lower(),
            }
        )
        v2_row = v2_manifest.get(record.image_id, {})
        v2_count = int(v2_row.get("detected_center_count", 0) or 0)
        comparison_rows.append(
            {
                "image_id": record.image_id,
                "source_filename": record.source_filename,
                "quality_flag": projection_row["quality_flag"],
                "v2_center_count": str(v2_count),
                "v3_center_count": str(len(accepted_centers)),
                "center_count_delta": str(len(accepted_centers) - v2_count),
                "v2_added_by_coverage_fallback_count": v2_row.get("added_by_coverage_fallback_count", ""),
                "v3_added_by_conditional_fallback_count": str(added_count),
                "v2_detection_quality_flag": v2_row.get("detection_quality_flag", ""),
                "v3_detection_quality_flag": quality,
                "fallback_allowed_v3": str(bool(detection_data["fallback_allowed"])).lower(),
                "fallback_reason_v3": str(detection_data["fallback_reason"]),
            }
        )
    ...
    reduced_count = sum(1 for row in comparison_rows if int(row["center_count_delta"]) < 0)
    print(f"Wrote {len(all_candidate_rows)} v3 lane-center candidates for {len(manifest_rows)} images")
    print(f"Wrote candidates to {CANDIDATES_PATH}")
    print(f"Wrote manifest to {MANIFEST_PATH}")
    print(f"Wrote v2-v3 comparison to {COMPARISON_PATH}; v3 reduced accepted center counts for {reduced_count} images")
