#!/usr/bin/env python3
"""Create original-image overlays for v5 adaptive lane-boundary candidates."""
...
CANDIDATES_PATH = PROJECT_ROOT / "outputs" / "lane_segmentation" / "lane_boundaries_v5" / "lane_boundary_candidates_v5.csv"
BOUNDARY_MANIFEST_PATH = PROJECT_ROOT / "outputs" / "lane_segmentation" / "lane_boundaries_v5" / "lane_boundary_manifest_v5.csv"
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "lane_segmentation" / "overlays" / "boundaries_v5"
OVERLAY_MANIFEST_PATH = OUTPUT_DIR / "lane_boundary_overlay_manifest_v5.csv"
...
def draw_label(draw: ImageDraw.ImageDraw, image_id: str, boundary_count: int, accepted_count: int, median_width: str, notes: str) -> None:
    label = f"{image_id}: {accepted_count}/{boundary_count} v5 adaptive lane boundaries; median width={median_width or 'n/a'} px"
    if notes:
        label = f"{label}; {notes}"
    try:
...
def create_overlay(image_id: str, candidate_rows: list[dict[str, str]], manifest_row: dict[str, str]) -> Path:
    ...
    for row in candidate_rows:
        ...
        draw.rectangle((left, 0, right, height - 1), fill=fill_color)
        draw.line((left, 0, left, height - 1), fill=BOUNDARY_EDGE_COLOR, width=line_width)
        draw.line((right, 0, right, height - 1), fill=BOUNDARY_EDGE_COLOR, width=line_width)
        draw.line((center_x, 0, center_x, height - 1), fill=line_color_for_candidate(row), width=line_width)
    ...
    output_path = OUTPUT_DIR / f"{image_id}_lane_boundary_overlay_v5.png"
...
def main() -> None:
    ...
    for record in records:
        ...
        notes = "v5 adaptive boundary overlay; yellow span=estimated lane coverage, yellow edges=estimated boundaries, red/orange/green/grey=center status"
    ...
    print(f"Wrote {len(manifest_rows)} v5 lane-boundary overlays to {OUTPUT_DIR}")
